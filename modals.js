...
function initializePatientSearchModal() {
    ...
    cpfInput.addEventListener('input', (e) => {
        ...
        // Enable button only if CPF is complete (11 digits)
        const cleanCpf = v.replace(/\D/g, '');
        searchBtn.disabled = cleanCpf.length !== 11;
    });
    
    searchBtn.addEventListener('click', () => {
        searchPatientBookings();
    });
...
function searchPatientBookings() {
    const cpfInput = document.getElementById('patientCpf');
...
function renderPatientBookingCard(dateKey, booking, period, status, months) {
    ...
    return `
        <div class="report-booking-card" style="margin-bottom: 12px;">
            <div class="report-booking-header">
                <strong>${day} de ${months[month]} de ${year}</strong>
                ${statusBadge}
            </div>
            <div class="report-booking-details">
                <p><strong>Período:</strong> ${periodName} (${periodTime})</p>
                <p><strong>Paciente:</strong> ${booking.name}</p>
                <p><strong>CPF:</strong> ${formattedCpf || 'N/A'}</p>
